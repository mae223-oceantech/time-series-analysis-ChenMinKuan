/*
  ...
  MODIFIED: Now uses GPIO 0 button instead of serial input to start NTRIP client
*/
#include <WiFi.h>
#include "secrets.h"
...
void loop()
{
  ...
  if (!ntripRunning) {
    Serial.println(F("Press button to start NTRIP Client. LED OFF = stopped"));
    delay(2000); // Don't spam the serial output
  }
}
